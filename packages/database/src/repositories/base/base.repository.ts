...
export abstract class BaseRepository<TTable extends PgTable, TEntity, TCreateDto, TUpdateDto> {
   ...
   * Default: spreads the DTO. Override for type conversions.
   */
  protected mapToInsertValues(dto: TCreateDto): Record<string, unknown> {
    return { ...(dto as Record<string, unknown>) }
  }

  /**
   * Maps an update DTO to database update values.
   * Default: copies only defined fields. Override for type conversions.
   */
  protected mapToUpdateValues(dto: TUpdateDto): Record<string, unknown> {
    return Object.fromEntries(
      Object.entries(dto as Record<string, unknown>).filter(([_, v]) => v !== undefined)
    )
  }
