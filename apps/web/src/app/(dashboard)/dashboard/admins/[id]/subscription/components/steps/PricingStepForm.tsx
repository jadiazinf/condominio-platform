...
import { DollarSign, Building2, Home, Calculator, Percent, Tag, AlertCircle, Info, RotateCcw } from 'lucide-react'
import { Button } from '@/ui/components/button'
import { useToast } from '@/ui/components/toast'
import { useTranslation } from '@/contexts'
import { useSubscriptionPricing, type IPricingQuery } from '@packages/http-client'
import type { ISubscriptionFormData, TDiscountType } from '../../hooks'
...
export function PricingStepForm({ companyId, shouldShowError, translateError }: PricingStepFormProps) {
  ...
  // Build pricing query based on discount settings (using debounced value)
  const pricingQuery = useMemo<IPricingQuery>(() => {
    ...
    return query
  }, [discountType, debouncedDiscountValue])

  // Fetch pricing data
  const { data: pricingData, isLoading: isPricingLoading, isError: isPricingError, error: pricingError } = useSubscriptionPricing(
    companyId,
    {
      query: pricingQuery,
      enabled: !!companyId,
  ...
  useEffect(() => {
    if (pricing) {
      ...
      setValue('rateId', pricing.rateId ?? null)
    ...
    <div className="space-y-8 pb-8">
                    ...
                    {isNoRateConfiguredError && (
                      <>
                        <Typography variant="body2" color="muted">
                          {t('superadmin.companies.subscription.form.pricing.noRateConfigured.description')}
                        </Typography>
                        <Typography variant="caption" color="muted" className="block">
                          {t('superadmin.companies.subscription.form.pricing.noRateConfigured.instruction')}
                        </Typography>
                      </>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-3 rounded-lg bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200/50 p-3 dark:from-emerald-900/20 dark:to-teal-900/10 dark:border-emerald-800/30">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <Typography variant="subtitle2" className="text-emerald-700 dark:text-emerald-300">
                          {t('superadmin.companies.subscription.form.pricing.rateApplied')}: {pricing.rateName}
                        </Typography>
                        {pricing.rateVersion && (
                    ...
                    <div className="text-right text-xs space-y-0.5">
                      <div className="text-default-500">
                        {t('superadmin.companies.subscription.form.pricing.condominiumRate')}: <span className="font-medium text-emerald-700 dark:text-emerald-300">{formatCurrency(pricing.condominiumRate)}</span>
                      </div>
                      <div className="text-default-500">
                        {t('superadmin.companies.subscription.form.pricing.unitRate')}: <span className="font-medium text-emerald-700 dark:text-emerald-300">{formatCurrency(pricing.unitRate)}</span>
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <div className="flex items-start gap-2">
                    <Tooltip
                      content={
                        <div className="max-w-xs space-y-2 p-1">
                          <p className="font-semibold">{t('superadmin.companies.subscription.form.pricing.formulaTooltip.title')}</p>
                          <ul className="text-xs space-y-1.5">
                            <li><span className="text-primary font-medium">{t('superadmin.companies.subscription.form.pricing.formulaTooltip.condominiums')}</span>: {t('superadmin.companies.subscription.form.pricing.formulaTooltip.condominiumsDesc')}</li>
                            <li><span className="text-primary font-medium">{t('superadmin.companies.subscription.form.pricing.formulaTooltip.condominiumRate')}</span>: {t('superadmin.companies.subscription.form.pricing.formulaTooltip.condominiumRateDesc')}</li>
                            <li><span className="text-secondary font-medium">{t('superadmin.companies.subscription.form.pricing.formulaTooltip.units')}</span>: {t('superadmin.companies.subscription.form.pricing.formulaTooltip.unitsDesc')}</li>
                            <li><span className="text-secondary font-medium">{t('superadmin.companies.subscription.form.pricing.formulaTooltip.unitRate')}</span>: {t('superadmin.companies.subscription.form.pricing.formulaTooltip.unitRateDesc')}</li>
                            <li><span className="text-success font-medium">{t('superadmin.companies.subscription.form.pricing.formulaTooltip.discount')}</span>: {t('superadmin.companies.subscription.form.pricing.formulaTooltip.discountDesc')}</li>
                          </ul>
                        </div>
                      }
                    >
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="font-mono text-xs bg-white/70 dark:bg-default-100/50 rounded px-3 py-2 mt-1 flex flex-wrap items-center gap-1">
                        ...
                        <span>) -</span>
                        <span className="text-success font-semibold">Descuento</span>
                  ...
                  <div className="flex items-center gap-3 rounded-lg bg-white/50 p-4 dark:bg-default-100/50">
                    ...
                    <div>
                      <div className="flex items-center gap-1">
                        ...
                        <Tooltip content={t('superadmin.companies.subscription.form.pricing.condominiumsDescription')}>
                  ...
                  <div className="flex items-center gap-3 rounded-lg bg-white/50 p-4 dark:bg-default-100/50">
                    ...
                    <div>
                      <div className="flex items-center gap-1">
                        ...
                        <Tooltip content={t('superadmin.companies.subscription.form.pricing.unitsDescription')}>
                          <Info size={12} className="text-default-400 cursor-help" />
                        </Tooltip>
                      </div>
                    ...
                    <Controller
                      ...
                      rules={{
                        validate: (value) => {
                          ...
                          if (numValue > 100) {
                            return t('superadmin.companies.subscription.form.validation.discount.maxPercentage')
                          }
                          return true
                        }
                      }}
                      render={({ field, fieldState }) => (
                        <Input
                          ...
                          placeholder={t('superadmin.companies.subscription.form.discount.percentagePlaceholder')}
                          type="number"
                          value={field.value}
                      ...
                      rules={{
                        validate: (value) => {
                          ...
                          if (pricing && numValue > pricing.calculatedPrice) {
                            return t('superadmin.companies.subscription.form.validation.discount.maxFixed')
                          }
                          return true
                        }
                      }}
      ...
      <div className="space-y-6">
        ...
        <Controller
          ...
          render={({ field }) => (
            <textarea
              placeholder={t('superadmin.companies.subscription.form.fields.pricingNotesPlaceholder')}
              value={field.value}
