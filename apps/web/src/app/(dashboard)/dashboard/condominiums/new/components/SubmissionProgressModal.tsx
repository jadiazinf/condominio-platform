...
export function SubmissionProgressModal({
  ...
  const isComplete =
    state.status === 'success' || state.status === 'partial_error' || state.status === 'error'

  const { progress } = state
  const totalSteps = 1 + progress.buildingsTotal + progress.unitsTotal
  const completedSteps =
    (progress.condominiumCreated ? 1 : 0) + progress.buildingsCreated + progress.unitsCreated
  const progressPercent = totalSteps > 0 ? (completedSteps / totalSteps) * 100 : 0

  return (
    <Modal hideCloseButton={!isComplete} isDismissable={false} isOpen={isOpen} onClose={onClose}>
      <ModalContent>
        <ModalHeader>
        ...
        <ModalBody>
          ...
          {state.errors.length > 0 && (
            <div className="mt-4 rounded-lg bg-danger-50 p-3">
              <Typography className="text-danger font-medium mb-1 block" variant="caption">
