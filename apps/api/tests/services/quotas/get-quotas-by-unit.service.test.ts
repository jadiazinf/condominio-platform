...
import { GetQuotasByUnitService } from '@src/services/quotas'

type TMockRepository = {
  getByUnitId: (unitId: string) => Promise<TQuota[]>
}

describe('GetQuotasByUnitService', function () {
  let service: GetQuotasByUnitService
    ...
    mockRepository = {
      getByUnitId: async function (requestedUnitId: string) {
        return mockQuotas.filter(function (q) {
          return q.unitId === requestedUnitId
        })
      },
    }
    service = new GetQuotasByUnitService(mockRepository as never)
  })

  describe('execute', function () {
    it('should return all quotas for a unit', async function () {
    ...
    it('should return empty array when unit has no quotas', async function () {
