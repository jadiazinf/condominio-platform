...
export class ChargesController extends BaseController<any, any, any> {
  ...
  private listCharges = async (c: Context): Promise<Response> => {
    ...
    const { unitId, condominiumId, periodYear, periodMonth } = ctx.query

    if (unitId && condominiumId && periodYear && periodMonth) {
      const charges = await this.chargesRepo.findByUnitAndPeriod(unitId, condominiumId, periodYear, periodMonth)
      return ctx.ok({ data: charges })
    }

    if (unitId && condominiumId) {
      const charges = await this.chargesRepo.findPendingByUnitAndCondominium(unitId, condominiumId)
      return ctx.ok({ data: charges })
    }
  ...
  private issueCreditNote = async (c: Context): Promise<Response> => {
    ...
    const creditNoteType = await this.chargeTypesRepo.findByCategory(charge.condominiumId, 'credit_note')
    if (!creditNoteType) {
  ...
  private issueDebitNote = async (c: Context): Promise<Response> => {
    ...
    const debitNoteType = await this.chargeTypesRepo.findByCategory(charge.condominiumId, 'debit_note')
    if (!debitNoteType) {
